...
def parting(folder_path):
    os.chdir(folder_path)
    for i in glob.glob(os.getcwd()+"/*.txt*"):
        file=i.split('\\')[-1]
        if re.search(r"_25kb",file):
            shutil.copyfile(i, '..\\res_25kb\\'+file)
        elif re.search(r"_100kb",file):
            shutil.copyfile(i, '..\\res_100kb\\'+file)
...
def list_tads(folder_path):
    l_tads=[]

    os.chdir(folder_path)
    for i in glob.glob(os.getcwd()+"/*.txt*"):
        l_tads+=read_consensus(i)
    return l_tads
